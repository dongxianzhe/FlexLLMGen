import numpy as np
import dataclasses
from flexllmgen.layer import Layer, InputEmbed, SelfAttention, MLP, TransformerLayer, OutputEmbed
from flexllmgen.utensor import ExecutionEnv
...
class OptModelFactory:
    def __init__(self, model_name: str, env: ExecutionEnv=None, policy=None):
        self.config = self._get_opt_config(model_name)
        self.env = env
        self.policy = policy

    def _get_opt_config(self, name: str):
        if "/" in name:
            name = name.split("/")[1]
        name = name.lower()
