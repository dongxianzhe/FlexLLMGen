...
from flexllmgen.utensor import DeviceType
from flexllmgen.utensor import get_global_disk_device, get_global_cpu_device
from flexllmgen.utils import torch_dtype_to_np_dtype, torch_dtype_to_num_bytes
    ...
    def load_from_np(self, np_array):
        if self.device.device_type == DeviceType.DISK:
            ...
            if self.device.device_type == DeviceType.COMPRESSED:
                ...
                tmp = get_global_cpu_device().compressed_device.compress(tmp, self.data[2])
...
def general_copy(dst: TorchTensor, dst_indices: Tuple[slice],
    ...
    if dst.device.device_type == DeviceType.MIXED:
        ...
        # as a relay
        get_global_disk_device().submit_copy(dst, dst_indices, src, src_indices)
